...
{
public:
    STM32CAN(CAN_HandleTypeDef *_hcan, BaseProtocol<U>& _base) : hcan(_hcan), base(_base) {};
    void InitFilter();
    void InitHW();
    void Tick(float Ts);
    void IRQHandler(const CAN_RxHeaderTypeDef& header, uint8_t *buffer);
private:
...
template<typename U>
void STM32CAN<U>::ProcessDataFrame(uint32_t id, uint8_t *payload, uint8_t len)
{
...
template<typename U>
void STM32CAN<U>::ProcessRemoteFrame(uint32_t id)
{
    CAN_OPCODES opcode = static_cast<CAN_OPCODES>(GetCANCommand(id));
    switch(opcode)
    {
...
template<typename U>
void STM32CAN<U>::Tick(float Ts)
{

}

template<typename U>
void STM32CAN<U>::IRQHandler(const CAN_RxHeaderTypeDef& header, uint8_t *buffer)
{
...
template<typename U>
void STM32CAN<U>::ConfigFilter(uint32_t id, uint32_t mask, uint8_t fifo_index, uint8_t filter_index)
{
...
template<typename U>
void STM32CAN<U>::SendPayload(uint32_t id, uint8_t *payload, uint8_t len)
{
    uint32_t mailbox = 0;
