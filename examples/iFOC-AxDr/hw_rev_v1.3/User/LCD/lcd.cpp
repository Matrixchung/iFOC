...
#else
	LCD_CS_Clr();
	uint8_t u8data[2] = {(uint8_t)(dat >> 8), (uint8_t)dat};
	HAL_SPI_Transmit(&hspi3, u8data, 2, 0xffff);
	LCD_CS_Set();
#endif
}
/**
...
{
	uint8_t sizex = sizey / 2;
    const int16_t num_int = num * mypow(10, decimal);
    ...
    for (uint8_t t = 0; t < len; t++)
    {
        ...
        const uint8_t temp = ((num_int / mypow(10, len - t - 1)) % 10) + '0';
        LCD_ShowChar(x + t * sizex, y, temp, fc, bc, sizey, 0);
    }
}
/**
