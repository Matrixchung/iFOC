...
// Credits: https://github.com/Left-Hand/Yumud/blob/master/core

#include "_IQNfunctions/support.hpp"
#include "_IQNfunctions/_IQNtoF.hpp"
#include "_IQNfunctions/_IQFtoN.hpp"
#include "_IQNfunctions/_IQNdiv.hpp"

#include "float/fp32.hpp"
#include "float/fp64.hpp"
...
namespace std{
    template<size_t Q>
    requires (Q < 32)
    struct is_arithmetic<iFOC::iq_t<Q>> : std::true_type {};

template<size_t Q>
requires (Q < 32)
struct is_floating_point<iFOC::iq_t<Q>> : std::false_type {};
}
...
#else
        __fast_inline constexpr iq_t(const float fv):value(__iqdetails::_IQFtoN<Q>(fv)){};
#endif
