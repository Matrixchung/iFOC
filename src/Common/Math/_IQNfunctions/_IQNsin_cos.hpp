...
__fast_inline static constexpr int_fast32_t __IQNcalcSin(const uint_fast32_t uiq31Input)
{
    uint_fast16_t index;
    int_fast32_t iq31X;
...
__fast_inline static constexpr int_fast32_t __IQNcalcCos(const uint_fast32_t uiq31Input)
{
    uint_fast16_t index;
    int_fast32_t iq31X;
    int_fast32_t iq31Sin;
...
__fast_inline static constexpr int_fast32_t __IQNsin_cos(int_fast32_t iqNInput, const int8_t q_value,
                                              const int8_t type, const int8_t format)
{
    ...
    if (format == TYPE_PU) {
        ...
        // }
        // 将输入直接转换为 Q29 格式
        uiq29Input = (uint32_t)iqNInput << (29 - q_value);

        // 直接计算模 2π
        uint32_t cycles = uiq29Input / _iq30_pi;
        uiq29Input -= cycles * _iq30_pi;

        // 处理符号
        ui8Sign = (cycles % 2) ? 1 : 0; // 奇偶周期符号翻转
