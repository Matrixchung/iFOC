...
public:
    ...
    void Update(float Ts) override
    {
        ...
        {
            shunt_values[0] = Ia_lpf.GetOutput((JDR_a->GetVoltage_mV() - zero_a) * current_factor_mV * (real_t)sign_a, Ts);
            shunt_values[2] = Ib_lpf.GetOutput((JDR_b->GetVoltage_mV() - zero_b) * current_factor_mV * (real_t)sign_b, Ts);
            shunt_values[1] = -shunt_values[0] - shunt_values[2];
        }
        else if constexpr (std::is_same_v<X, V> && std::is_same_v<Y, W>) // Phase V(a), W(b)
        {
            shunt_values[1] = Ia_lpf.GetOutput((JDR_a->GetVoltage_mV() - zero_a) * current_factor_mV * (real_t)sign_a, Ts);
            shunt_values[2] = Ib_lpf.GetOutput((JDR_b->GetVoltage_mV() - zero_b) * current_factor_mV * (real_t)sign_b, Ts);
