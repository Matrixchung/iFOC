...
void CurrLoopPI::UpdateRT(float Ts)
{
    const auto foc = GetMotor<FOCMotor>();
    if(foc->IsArmed())
    {
        ...
        // Ud = Rs * Id - We * Lq * Iq
        const bool enable_pi_feedforward = foc->GetConfig().phase_inductance_valid();
