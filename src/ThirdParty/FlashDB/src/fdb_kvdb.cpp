...
/* GC minimum number of empty sectors. GC will use at least 1 empty sector. */
#define GC_MIN_EMPTY_SEC_NUM                     1

/* the sector remains threshold before full status */
...
static bool find_kv_cb(fdb_kv_t kv, void *arg1, void *arg2)
{
    ...
    bool *find_ok = static_cast<bool *>(arg2);
...
#ifdef FDB_KV_USING_CACHE
        {
//            struct kvdb_sec_info sector = {.check_ok = false, .addr = addr, .empty_kv = FAILED_ADDR };
            struct kvdb_sec_info sector{};
            sector.check_ok = false;
            sector.addr = addr;
            sector.empty_kv = FAILED_ADDR;
...
{
    size_t *empty_sector = static_cast<size_t *>(arg1), *using_sector = static_cast<size_t *>(arg2);
...
{
    struct alloc_kv_cb_args *arg = static_cast<alloc_kv_cb_args *>(arg1);
...
{
    size_t *empty_sec_num = static_cast<size_t *>(arg1);
    uint32_t *empty_sec_addr = static_cast<uint32_t *>(arg2);
...
{
    if (!sector->check_ok) {
        size_t *failed_count = static_cast<size_t *>(arg1);
        fdb_kvdb_t db = static_cast<fdb_kvdb_t>(arg2);
...
{
    fdb_kvdb_t db = static_cast<fdb_kvdb_t>(arg1);
