...
bool uavcan_protocol_dynamic_node_id_Allocation_decode(const DroneCAN::CanardRxTransfer* transfer, struct uavcan_protocol_dynamic_node_id_Allocation* msg);
...
static inline bool _uavcan_protocol_dynamic_node_id_Allocation_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_dynamic_node_id_Allocation* msg, bool tao);
void _uavcan_protocol_dynamic_node_id_Allocation_encode(uint8_t* buffer, uint32_t* bit_ofs, struct uavcan_protocol_dynamic_node_id_Allocation* msg, bool tao) {
    ...
    *bit_ofs += 7;
    DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 1, &msg->first_part_of_unique_id);
    *bit_ofs += 1;
...
#pragma GCC diagnostic pop
    if (!tao) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 5, &unique_id_len);
        *bit_ofs += 5;
    }
    for (size_t i=0; i < unique_id_len; i++) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 8, &msg->unique_id.data[i]);
        *bit_ofs += 8;
    }
}
...
*/
bool _uavcan_protocol_dynamic_node_id_Allocation_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_dynamic_node_id_Allocation* msg, bool tao) {
    (void)transfer;
    (void)bit_ofs;
    (void)msg;
    (void)tao;
    ...
    *bit_ofs += 7;
    ...
    DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 1, false, &msg->first_part_of_unique_id);
    *bit_ofs += 1;

    if (!tao) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 5, false, &msg->unique_id.len);
        *bit_ofs += 5;
    } else {
...
#pragma GCC diagnostic pop
    for (size_t i=0; i < msg->unique_id.len; i++) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->unique_id.data[i]);
        *bit_ofs += 8;
    }
