...
bool uavcan_protocol_dynamic_node_id_server_Discovery_decode(const DroneCAN::CanardRxTransfer* transfer, struct uavcan_protocol_dynamic_node_id_server_Discovery* msg);
...
static inline bool _uavcan_protocol_dynamic_node_id_server_Discovery_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_dynamic_node_id_server_Discovery* msg, bool tao);
void _uavcan_protocol_dynamic_node_id_server_Discovery_encode(uint8_t* buffer, uint32_t* bit_ofs, struct uavcan_protocol_dynamic_node_id_server_Discovery* msg, bool tao) {
    ...
    DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 8, &msg->configured_cluster_size);
    *bit_ofs += 8;
...
#pragma GCC diagnostic pop
    if (!tao) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 3, &known_nodes_len);
        *bit_ofs += 3;
    }
    for (size_t i=0; i < known_nodes_len; i++) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 8, &msg->known_nodes.data[i]);
        *bit_ofs += 8;
    }
}
...
*/
bool _uavcan_protocol_dynamic_node_id_server_Discovery_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_dynamic_node_id_server_Discovery* msg, bool tao) {
    (void)transfer;
    (void)bit_ofs;
    (void)msg;
    (void)tao;
    DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->configured_cluster_size);
    *bit_ofs += 8;

    if (!tao) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 3, false, &msg->known_nodes.len);
        *bit_ofs += 3;
    } else {
...
#pragma GCC diagnostic pop
    for (size_t i=0; i < msg->known_nodes.len; i++) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->known_nodes.data[i]);
        *bit_ofs += 8;
    }
