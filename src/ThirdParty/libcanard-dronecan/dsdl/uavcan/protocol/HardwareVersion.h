...
bool uavcan_protocol_HardwareVersion_decode(const DroneCAN::CanardRxTransfer* transfer, struct uavcan_protocol_HardwareVersion* msg);
...
static inline bool _uavcan_protocol_HardwareVersion_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_HardwareVersion* msg, bool tao);
...
#pragma GCC diagnostic pop
    if (!tao) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 8, &certificate_of_authenticity_len);
        *bit_ofs += 8;
    }
    for (size_t i=0; i < certificate_of_authenticity_len; i++) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 8, &msg->certificate_of_authenticity.data[i]);
        *bit_ofs += 8;
    }
}
...
*/
bool _uavcan_protocol_HardwareVersion_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_HardwareVersion* msg, bool tao) {
    (void)transfer;
    (void)bit_ofs;
    (void)msg;
    (void)tao;
    ...
    *bit_ofs += 8;
    ...
    if (!tao) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->certificate_of_authenticity.len);
        *bit_ofs += 8;
    } else {
...
#pragma GCC diagnostic pop
    for (size_t i=0; i < msg->certificate_of_authenticity.len; i++) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->certificate_of_authenticity.data[i]);
        *bit_ofs += 8;
    }
