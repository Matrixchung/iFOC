...
bool uavcan_protocol_Panic_decode(const DroneCAN::CanardRxTransfer* transfer, struct uavcan_protocol_Panic* msg);
...
static inline bool _uavcan_protocol_Panic_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_Panic* msg, bool tao);
...
#pragma GCC diagnostic pop
    if (!tao) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 3, &reason_text_len);
        *bit_ofs += 3;
    }
    for (size_t i=0; i < reason_text_len; i++) {
        DroneCAN::canardEncodeScalar(buffer, *bit_ofs, 8, &msg->reason_text.data[i]);
        *bit_ofs += 8;
    }
}
...
*/
bool _uavcan_protocol_Panic_decode(const DroneCAN::CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_Panic* msg, bool tao) {
    (void)transfer;
    (void)bit_ofs;
    (void)msg;
    (void)tao;
    if (!tao) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 3, false, &msg->reason_text.len);
        *bit_ofs += 3;
    } else {
...
#pragma GCC diagnostic pop
    for (size_t i=0; i < msg->reason_text.len; i++) {
        DroneCAN::canardDecodeScalar(transfer, *bit_ofs, 8, false, &msg->reason_text.data[i]);
        *bit_ofs += 8;
    }
